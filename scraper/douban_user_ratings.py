...
def parse_collect_block(block):
    ...
    vote_tag = block.select_one('span.pl')
    vote_useful = vote_tag.text.strip() if vote_tag else None
...
def main_loop():
    ...
    total_page, first_page_drama =  fetch_collect_page(0, headers=request_headers, user_id=278582560)
 
    all_drama = first_page_drama
    for page in range(1, total_page):
        try:
            _, drama = fetch_collect_page(page, headers=request_headers, user_id=278582560)
